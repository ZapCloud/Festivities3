...
public class TileEntityPlate extends TileEntityFestive
{
	...
	public static enum PlateFoods
	{
		...
		PmkPie(Items.pumpkin_pie, 0, false),
		ChipCookie(Items.cookie, 0, true),
		...
		{
			return getFood(item.getItem(), item.getItemDamage());
		}

		public static PlateFoods getFood(Item item, int meta)
		{
			for (int i = 0; i < PlateFoods.values().length; i++)
			{
				PlateFoods f = PlateFoods.values()[i];
				if (f.item == item && f.meta == meta)
				{
					return f;
				}
			}
